...
verify_new_fac_spellings <- function(
    ...
    out <- new_fac_spellings %>% 
        ...
        mutate(exists_ = is_fac_name(xwalk_name_clean, State, old_fac_info, old_fac_spellings, include_alt = FALSE),
               ...
               valid_federal_flag_ = is_valid_flag(Is.Federal), 
               drop_ = ifelse(exists_ == FALSE # Drop if does NOT already exist in fac_info 
                              | valid_state_ == FALSE  
                              | valid_federal_flag_ == FALSE, 
                              TRUE, FALSE))
    ...
    if (ndrops > 0) {
        ...
        for(i in 1:nrow(out)) {
            ...
            if (!row$valid_state_) {
                get_custom_warning("State name is invalid:", row, row$State)}
            if (!row$valid_federal_flag_) {
                get_custom_warning("Federal flag is invalid:", row, row$Is.Federal)}
            }}
...
populate_new_spellings <- function(
    ...
    # Merge federal facilities on cleaned name 
    federal <- new_fac_spellings %>% 
        ...
        left_join(old_fac_info %>% 
                  ...
                  by = "name_clean_") %>% 
        select(Facility.ID, xwalk_name_raw, xwalk_name_clean, State, Is.Federal, Source)
